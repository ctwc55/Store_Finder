...
vector<Node> print_route; //경로 출력에 사용되는 스택
...
int cvsPage();
int cafePage();
void printPossibleListPage(int print_type);
...
int restaurantPage() {
	...
	//출력 및 url 연결
	if (print_type == 8) printPossibleListPage(0); //TOP 5
	else if (print_type == 9) printPossibleListPage(1); //전체
	...
	while (run) {
		...
		case SPACE:
			if (y == 7) {
				...
				else if (x == 33) return 99;//처음으로
				else if (x == 48) return 99;//뭐하지
			}
			run = FALSE;
			break;
...
int drugstorePage() { //사용자 현재위치 선택
	int x = 3, y = 8, user_position, com, run = TRUE;
...
void filteringStore(int store_type, int restaurant_type, int area, int price) {
	//음식 종류 : 한식 = 1, 일식 = 2, 중식 = 3, 양식 = 4, 고기 = 5, 닭 = 6, 기타 = 7
	//가게 위치 : 교내 = 0, 쪽문 = 1, 대명거리 = 2, 정문 = 3, 소나무길 = 4, 대학로 = 5
	...
	for (int i = 0; i < STORENUM; i++) {
		if (store_type == RESTAURANT) {
			if (store[i].type_eat == restaurant_type) {
				if (area == 0 && (i >= 0 && i <= 6)) {
